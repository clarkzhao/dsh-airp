...
export function interviewScreens(): [InterviewCard, InterviewCard] {
...
export function interviewCard(screen?: 1 | 2): InterviewCard {
  if (screen === 1) return interviewScreens()[0]
  if (screen === 2) return interviewScreens()[1]
  return { questions: interviewQuestions() }
}

export function mergeInterviewAnswers(
  ...chunks: Array<{ answers?: Array<{ id?: string; selected?: string[]; custom?: string }> } | InterviewAnswers | undefined>
): InterviewAnswers {
  return chunks.reduce<InterviewAnswers>((acc, chunk) => {
    if (!chunk) return acc
    return { ...acc, ...parseInterview(chunk) }
  }, {})
}
