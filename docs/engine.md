...
- 没有 `webServer` 时 `markdownUrl` 为 `undefined`，brief 写「当前没有 Web 舞台」，不要贴路径。Host 用 `ctx.inject(['webServer'])` 挂 `/airp-media`，不要在 `apply()` 里对 `ctx.get('webServer')` 做一次性探测——插件可能比 Web 服务先激活。
